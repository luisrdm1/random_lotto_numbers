...
fn bench_single_ticket(c: &mut Criterion) {
    ...
    for (name, start, end, pick) in scenarios {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(name), &(range, pick_count), 
            |b, (range, pick)| {
                let mut rng = rand::rng();
                b.iter(|| {
                    generate_ticket(black_box(&mut rng), black_box(range), black_box(pick))
                });
            }
        );
    }
...
fn bench_ticket_strategies(c: &mut Criterion) {
    ...
    group.bench_function("insertion_strategy_6_from_60", |b| {
        let mut rng = rand::rng();
        b.iter(|| {
            generate_ticket(black_box(&mut rng), black_box(&range_insertion), black_box(&pick_insertion))
        });
    });
    ...
    group.bench_function("exclusion_strategy_50_from_100", |b| {
        let mut rng = rand::rng();
        b.iter(|| {
            generate_ticket(black_box(&mut rng), black_box(&range_exclusion), black_box(&pick_exclusion))
        });
    });
...
criterion_group!(benches, bench_single_ticket, bench_unique_tickets, bench_ticket_strategies);
