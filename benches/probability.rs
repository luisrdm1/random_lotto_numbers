...
fn bench_combination(c: &mut Criterion) {
    ...
    for (name, n, k) in scenarios {
        group.bench_with_input(BenchmarkId::from_parameter(name), &(n, k), |b, &(n, k)| {
            b.iter(|| combination(black_box(n), black_box(k)).unwrap());
        });
    }
...
fn bench_probability(c: &mut Criterion) {
    ...
    for (name, total, pick, match_count) in scenarios {
        group.bench_with_input(
            ...
            |b, &(t, p, m)| {
                b.iter(|| calculate_probability(black_box(t), black_box(p), black_box(m)).unwrap());
            },
        );
    }
...
fn bench_combination_edge_cases(c: &mut Criterion) {
    ...
    group.bench_function("C(100, 5) - small k", |b| {
        b.iter(|| combination(black_box(100), black_box(5)).unwrap());
    });

    group.bench_function("C(100, 95) - large k (uses symmetry)", |b| {
        b.iter(|| combination(black_box(100), black_box(95)).unwrap());
    });

    // Test edge cases
    group.bench_function("C(1000, 1) - trivial", |b| {
        b.iter(|| combination(black_box(1000), black_box(1)).unwrap());
    });

    group.bench_function("C(100, 0) - zero", |b| {
        b.iter(|| combination(black_box(100), black_box(0)).unwrap());
    });
